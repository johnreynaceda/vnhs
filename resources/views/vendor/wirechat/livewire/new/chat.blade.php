...
<div id="new-chat-modal">
    <div class="relative w-full h-96 border mx-auto overflow-auto bg-white text-black px-7 sm:max-w-lg sm:rounded-lg">
        <header class="sticky top-0 bg-white z-10 py-2">
            <div class="flex justify-between items-center pb-2">
                ...
                <x-wirechat::actions.close-modal>
                    <button dusk="close_modal_button"
                        class="p-2 text-gray-600 hover:text-gray-800 hover:bg-gray-50 rounded-full">
            ...
            <section class="flex flex-wrap items-center px-0 border-b">
                <input dusk="search_users_field" autofocus type="search" id="users-search-field"
                    ...
                    class="w-full border-0 px-0 bg-white outline-none focus:outline-none rounded-lg focus:ring-0">
            </section>
        </header>
            ...
            @if (WireChat::showNewGroupModalButton() && auth()->user()->canCreateGroups())
                <x-wirechat::actions.new-group widget="{{ $this->isWidget() }}">
                    <button @dusk="open_new_group_modal_button"
                        class="flex items-center gap-3 my-4 rounded-lg p-2 w-full border hover:border-gray-300 transition-colors">
                        ...
                        <p class="text-black">@lang('wirechat::new.chat.actions.new_group.label')</p>
                    </button>
                    ...
                    <ul class="overflow-auto flex flex-col">
                        @foreach ($users as $key => $user)
                            <li wire:key="user-{{ $key }}"
                                ...
                                class="flex cursor-pointer group gap-2 items-center p-2 hover:bg-gray-100 rounded">
